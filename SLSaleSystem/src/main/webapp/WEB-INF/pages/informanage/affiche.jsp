...
<div>
	<ul class="breadcrumb">
		...
		<li><a href="/SLSaleSystem/informanage/affiche.html">公告管理</a></li>
	</ul>
</div>
				...
				<div class="box span12">
					...
					<div class="pagination pagination-centered">
					  ...
					  <c:choose>
					  	...
					  	<c:otherwise>
					  	<li><a href="/SLSaleSystem/informanage/affiche.html?p=1" title="首页">首页</a></li>
					  	</c:otherwise>
					  </c:choose>
						
						<c:if test="${page.prevPages!=null}">
							<c:forEach items="${page.prevPages}" var="num">
								<li><a href="/SLSaleSystem/informanage/affiche.html?p=${num}"
						...
						<c:if test="${page.nextPages!=null}">
							<c:forEach items="${page.nextPages}" var="num">
								<li><a href="/SLSaleSystem/informanage/affiche.html?p=${num}" title="${num}">
								${num} </a></li>
							</c:forEach>
						</c:if>
						<c:if test="${page.pageCount !=null}">
							<c:choose>
						  	...
						  	<c:otherwise>
						  	<li><a href="/SLSaleSystem/informanage/affiche.html?p=${page.pageCount}" title="尾页">尾页</a></li>
						  	</c:otherwise>
						    </c:choose>
					    </c:if>
						<c:if test="${page.pageCount == null}">
	...
	<div class="modal hide fade" id="addAfficheDiv">
		<form action="/SLSaleSystem/informanage/addAffiche.html" enctype="multipart/form-data" method="post" onsubmit="return addAfficheFunction();">
	 ...
	 <div class="modal hide fade" id="modifyAfficheDiv">
		<form action="/SLSaleSystem/informanage/modifyAffiche.html" enctype="multipart/form-data" method="post" onsubmit="return modifyAfficheFunction();">
			<div class="modal-header">
				<button type="button" class="close addusercancel" data-dismiss="modal">×</button>
				<h3>修改公告信息</h3>
...
<script type="text/javascript" src="<%=path %>/statics/localjs/affiche.js"></script> 
