...
$('.delmessage').click(function(e){
	...
	if(confirm("您确定要删除用户:【"+d_createdBy+"】的该条留言吗？")){
		//delete
		$.post("/SLSaleSystem/backend/delmessage.html",{'delId':d_id},function(result){
			if("success" == result){
				alert("删除成功！");
				window.location.href="/SLSaleSystem/message/messagelist.html";
			}else{
				alert("删除失败！");
			}
		},'html');
...
$('.replymessage').click(function(e){
	...
	$.ajax({
		url: '/SLSaleSystem/message/getmessage.html',
		type: 'POST',
		data:{id:r_id},
		dataType: 'html',
		timeout: 1000,
