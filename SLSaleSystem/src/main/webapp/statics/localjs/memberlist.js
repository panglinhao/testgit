...
$('.delmember').click(function(e){
	...
	if(confirm("您确定要删除【"+d_logincode+"】这个用户吗？")){
		//delete
		$.post("/SLSaleSystem/backend/deluser.html",{'delId':d_id,'delIdCardPicPath':d_idcardpicpath,'delBankPicPath':d_bankpicpath,'delUserType':'1'},function(result){
			if("success" == result){
				alert("删除成功！");
				window.location.href="/SLSaleSystem/member/memberlist.html";
			}else{
				alert("删除失败！");
			}
		},'html');
