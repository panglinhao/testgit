...
$("#modifytypeCode").blur(function(){
	...
	if(mlc != ""){
		$.post("/SLSaleSystem/backend/typecodeisexit.html",{'typeCode':mlc,'id':$("#modifydicid").val()},function(result){
			if(result == "repeat"){
...
$(".typecodelist").click(function(){
	...
	$.ajax({
		url:"/SLSaleSystem/backend/getJsonDic.html",
		type: 'POST',
		data:{typeCode:typecode},
		dataType: 'html',
		timeout: 5000,
		error: function(){
			alert("加载"+typename+"失败!请重试。");
		},
			...
			if("failed" == result){
				...
				$(".modifyDicValue").click(function(){
					...
					if(confirm("您确定要修改【"+modify.attr("modifytypename")+"】- 【"+dic.valueName+"】- 【"+dic.valueId+"】的数据配置吗？")){
						...
						$.ajax({
							url: '/SLSaleSystem/backend/modifyDic.html',
							type: 'POST',
							...
							success: function(result){
								if(result != "" && "success" == result){
									...
									tip.css("color","red");
									tip.html("添加失败！该类型代码下的数据名称不能重复，请重试。");
								}
							}
							});
						}
				...
				$(".delDicValue").click(function(){
					...
					if(confirm("您确定要删除【"+del.attr("deltypename")+"】- 【"+dic.valueName+"】- 【"+dic.valueId+"】的数据配置吗？")){
						//delDic id
						$.ajax({
							url: '/SLSaleSystem/backend/delDic.html',
							type: 'POST',
							data:{id:dic.id},
							dataType: 'html',
							timeout: 1000,
							...
							success: function(result){
								if(result != "" && "success" == result){
									...
									$("#dicListUL").children("#li"+dic.id).remove();
...
$("#modifyDicExeBtn").click(function(){
	...
	if(dic.typeCode == ""){
		...
		$.ajax({
			url: '/SLSaleSystem/backend/modifylDic.html',
			type: 'POST',
			...
			success: function(result){
				if(result != "" && "success" == result){
					window.location.href = "/SLSaleSystem/backend/dicmanage.html";
				}else if("failed" == result){
					tip.css("color","red");
					tip.html("数据字典修改失败！请重试。");
				}else if("nodata" == result){
...
$(".delMainDic").click(function(){
	...
	if(confirm("删除操作会删除\""+dictypename+"\"下所有的字典值，您确定要删除\""+dictypename+"\"吗？")){
		...
		$.ajax({
			url: '/SLSaleSystem/backend/delMainDic.html',
			type: 'POST',
...
$("#addDicExeBtn").click(function(){
	...
	if(dic.typeCode == ""){
		...
		$.ajax({
			url: '/SLSaleSystem/backend/addDic.html',
			type: 'POST',
			data:{dic:JSON.stringify(dic)},
			dataType: 'html',
			timeout: 1000,
...
$("#addDicsubExeBtn").click(function(){
	...
	if(dic.typeCode == ""){
		...
		$.ajax({
			url: '/SLSaleSystem/backend/addDicSub.html',
			type: 'POST',
			data:{dic:JSON.stringify(dic)},
			dataType: 'html',
			timeout: 1000,
			...
			success: function(result){
				if("failed" == result){
					...
					tip.css("color","red");
					tip.html("数据字典添加失败！该类型代码下的数据名称不能重复，请重试。");
				}else if("nodata" == result){
					alert("对不起，没有任何数据需要处理！请重试。");
					...
					$("a[typeCode='"+dic.typeCode+"']").trigger("click");
					tip.css("color","green");
					tip.html("数据字典添加成功。^_^。");
				}
			}
