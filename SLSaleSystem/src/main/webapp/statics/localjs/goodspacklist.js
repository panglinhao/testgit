$(".addgoodspack").click(function(){
	window.location.href="/SLSaleSystem/backend/addgoodspack.html";
});

$('.modifygoodspack').click(function(e){
	var id = $(this).attr('id');
	window.location.href="/SLSaleSystem/backend/modifygoodspack.html?id="+id;
});

$('.viewgoodspack').click(function(e){
	var id = $(this).attr('id');
	window.location.href="/SLSaleSystem/backend/viewgoodspack.html?id="+id;
});
...
$('.delgoodspack').click(function(e){
	...
	if(confirm("您确定要删除【"+goodsPackName+"】这个商品套餐吗？")){
		//delete
		$.post("/SLSaleSystem/backend/delgoodspack.html",{'delId':d_id},function(result){
			if("success" == result){
				alert("删除成功！");
...
$(".modifystate").click(function(){
	...
	$.ajax({
		url: '/SLSaleSystem/backend/modifygoodspackstate.html',
		type: 'POST',
