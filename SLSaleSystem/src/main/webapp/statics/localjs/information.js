...
function delFile(){
	$.ajax({
		url: '/SLSaleSystem/informanage/delInfoFile.html',
		type: 'POST',
...
{   
	if($("#uploadInformationFile").val() == "" || $("#uploadInformationFile").val()  == null){
		...
		$.ajaxFileUpload
	    ({ 
	           url:'/SLSaleSystem/informanage/upload.html', //处理上传文件的服务端
	           secureuri:false,
...
$(".viewinformation").click(function(e){
	...
	$.ajax({
		url: '/SLSaleSystem/informanage/viewInfo.html',
		type: 'POST',
		data:{id:id},
		dataType: 'html',
		timeout: 1000,
...
$(".delinformation").click(function(){
	...
	if(confirm("您确定要删除" + title + "吗？" )){
		$.ajax({
			url: '/SLSaleSystem/informanage/delInfo.html',
			type: 'POST',
			data:{id:id},
			dataType: 'html',
			timeout: 1000,
			...
			success: function(result){
				if(result != "" && "success" == result){
					window.location.href="/SLSaleSystem/informanage/information.html";
				}else if("failed" == result){
					alert("删除"+title+"失败！请重试。");
				}else if("nodata" == result){
					alert("对不起，没有任何数据需要处理！请重试。");
...
$(".modifyInformationState").click(function(){
	...
	$.ajax({
		url: '/SLSaleSystem/informanage/modifyInfoState.html',
		type: 'POST',
